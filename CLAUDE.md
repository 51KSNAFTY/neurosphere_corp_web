...
src/
  ...
  lib/
    ...
    jsonld.ts             # JSON-LD構造化データ（Organization）
  types/
    global.d.ts           # グローバル型定義（fbq等）
  styles/
...
- **Meta Pixel（Facebook Pixel）**: `src/components/meta-pixel.tsx` でルートレイアウトに統合。全ページでトラッキング有効
- **SSGform**: お問い合わせフォーム（`src/components/sections/contact-form.tsx`）で使用。静的サイト向けフォームバックエンド
- **JSON-LD**: `src/lib/jsonld.ts` でOrganization構造化データを定義、`layout.tsx`で全ページに埋め込み
